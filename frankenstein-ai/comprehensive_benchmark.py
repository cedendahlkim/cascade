"""
Comprehensive Benchmark: Frankenstein AI vs Ren Gemini 2.0 Flash

Testar ALLA relevanta områden:
- 20+ uppgiftskategorier (arithmetic → backtracking)
- Svårighetsnivåer 1-10
- V2-uppgifter (bugfix, api_design, optimization)
- Mäter: solve rate, first-try rate, tid, strategianvändning

Resultat sparas lokalt + laddas upp till Cascade Remote bridge
för nedladdning via mobilen.

Kör: python comprehensive_benchmark.py [--tasks-per-category 3] [--bridge-url http://localhost:3031]
"""
...
# All standard categories mapped to their difficulty levels
...
# V2 categories (software engineering)
V2_CATEGORIES = ["bugfix", "api_design", "optimization", "test_writing", "code_review"]


class GeminiAgent:
    """Ren Gemini 2.0 Flash agent — ingen Frankenstein-stack."""
    ...
    def solve_task(self, task: Task) -> dict:
        """Solve a task and return detailed result dict."""
        self.total_tasks += 1
        cat = task.category
...
class BenchmarkResult:
    """Result for a single task comparison."""
    task_id: str
    title: str
    difficulty: int
    category: str
...
def _upload_to_bridge(bridge_url: str, data: dict, filename: str) -> str | None:
    """Upload JSON result to bridge file-sharing for mobile download."""
    try:
        ...
        resp = requests.post(
            ...
            json={
                ...
                "filename": filename,
                "uploadedBy": "ai",
                "description": f"Comprehensive Benchmark: Frankenstein AI vs Gemini 2.0 Flash — {data.get('summary', {}).get('total_tasks', '?')} tasks",
                "tags": ["benchmark", "frankenstein", "gemini", "ab-test"],
            },
            timeout=10,
        )
        if resp.status_code == 200:
...
def _generate_markdown_report(data: dict) -> str:
    ...
    lines = [
        f"# Frankenstein AI vs Gemini 2.0 Flash — Benchmark",
        f"",
        f"**Datum:** {data.get('timestamp', '?')}",
        f"**Uppgifter:** {s.get('total_tasks', 0)}",
        f"**Kategorier:** {s.get('categories_tested', 0)}",
        f"**Svårighetsnivåer:** {s.get('difficulty_range', '?')}",
        f"",
        f"## Sammanfattning",
        f"",
        f"| Metrik | Frankenstein AI | Gemini 2.0 Flash | Skillnad |",
        f"|---|---|---|---|",
        f"| Lösta | {f.get('solved', 0)}/{s.get('total_tasks', 0)} ({f.get('solve_rate', 0):.0%}) | {g.get('solved', 0)}/{s.get('total_tasks', 0)} ({g.get('solve_rate', 0):.0%}) | {s.get('diff_solve_rate', 0):+.1%} |",
        ...
        f"",
        f"**Vinnare: {s.get('winner_text', '?')}**",
        f"",
        f"## Per svårighetsnivå",
        f"",
        f"| Nivå | Frank | Gemini | Diff |",
        f"|---|---|---|---|",
    ]

    for lvl_str, lvl_data in sorted(data.get("per_level", {}).items(), key=lambda x: int(x[0])):
        fd = lvl_data.get("frankenstein", {})
        gd = lvl_data.get("gemini", {})
        n = fd.get("total", 0)
        fr = fd.get("rate", 0)
        gr = gd.get("rate", 0)
        diff = fr - gr
        emoji = "🧟" if diff > 0 else ("🤖" if diff < 0 else "🤝")
        lines.append(f"| Nivå {lvl_str} | {fd.get('solved', 0)}/{n} ({fr:.0%}) | {gd.get('solved', 0)}/{n} ({gr:.0%}) | {diff:+.0%} {emoji} |")

    lines += [
        f"",
        f"## Per kategori",
        f"",
        f"| Kategori | Nivå | Frank | Gemini | Diff |",
        f"|---|---|---|---|---|",
    ]

    for cat_data in sorted(data.get("per_category", {}).values(), key=lambda x: x.get("difficulty", 0)):
        cat = cat_data.get("category", "?")
        diff_lvl = cat_data.get("difficulty", 0)
...
def run_comprehensive_benchmark(
    ...
    """Run a comprehensive benchmark across all categories and difficulty levels."""

    print("=" * 72)
    print("  COMPREHENSIVE BENCHMARK: Frankenstein AI vs Gemini 2.0 Flash")
    print("=" * 72)
    ...
    print(f"  Standard categories: {len(CATEGORY_DIFFICULTY_MAP)}")
    print(f"  V2 categories: {len(V2_CATEGORIES) if include_v2 else 0}")
    total_expected = tasks_per_category * (len(CATEGORY_DIFFICULTY_MAP) + (len(V2_CATEGORIES) if include_v2 else 0))
    ...
    print("\n  Generating tasks...")
    all_tasks: list[tuple[Task, str]] = []  # (task, source)

    for cat, diff in CATEGORY_DIFFICULTY_MAP.items():
        for _ in range(tasks_per_category):
            task = generate_task(diff)
            all_tasks.append((task, "standard"))

    if include_v2:
        for _ in range(tasks_per_category * len(V2_CATEGORIES)):
            v2_diff = random.randint(4, 8)
            try:
                task = generate_v2_task(v2_diff)
                all_tasks.append((task, "v2"))
            except Exception:
                pass
    ...
    _send_event(bridge_url, {
        ...
        "categories": list(CATEGORY_DIFFICULTY_MAP.keys()) + (V2_CATEGORIES if include_v2 else []),
    })
    ...
    frank_avg_ms = sum(r.frank_time_ms for r in results) / max(total, 1)
    gemini_avg_ms = sum(r.gemini_time_ms for r in results) / max(total, 1)
    frank_rate = frank_total_solved / max(total, 1)
    gemini_rate = gemini_total_solved / max(total, 1)
    diff_rate = frank_rate - gemini_rate

    if diff_rate > 0.05:
        winner_text = f"Frankenstein AI vinner med {diff_rate:.1%} hogre losningsgrad!"
    elif diff_rate < -0.05:
        winner_text = f"Gemini 2.0 Flash vinner med {-diff_rate:.1%} hogre losningsgrad."
    else:
        winner_text = f"Oavgjort (skillnad: {diff_rate:+.1%})"

    # Per level
    per_level = {}
    for lvl in sorted(set(r.difficulty for r in results)):
        lr = [r for r in results if r.difficulty == lvl]
        n = len(lr)
        fs = sum(1 for r in lr if r.frank_solved)
        gs = sum(1 for r in lr if r.gemini_solved)
        per_level[str(lvl)] = {
            "frankenstein": {"solved": fs, "total": n, "rate": round(fs / max(n, 1), 3)},
            "gemini": {"solved": gs, "total": n, "rate": round(gs / max(n, 1), 3)},
        }
    ...
    for cat in sorted(set(r.category for r in results)):
        ...
        per_category[cat] = {
            "category": cat,
            "difficulty": cr[0].difficulty if cr else 0,
            "frankenstein": {"solved": fs, "total": n, "rate": round(fs / max(n, 1), 3), "first_try": fft},
            "gemini": {"solved": gs, "total": n, "rate": round(gs / max(n, 1), 3), "first_try": gft},
        }
    ...
    output = {
        ...
        "summary": {
            ...
            "frankenstein": {
                ...
                "first_try": frank_ft,
                "first_try_rate": round(frank_ft / max(total, 1), 3),
            ...
            "gemini": {
                ...
                "first_try": gemini_ft,
                "first_try_rate": round(gemini_ft / max(total, 1), 3),
                "avg_time_ms": round(gemini_avg_ms, 1),
            },
            "diff_solve_rate": round(diff_rate, 3),
            "diff_first_try_rate": round((frank_ft - gemini_ft) / max(total, 1), 3),
    ...
    # === PRINT REPORT ===
    print("\n" + "=" * 72)
    print("  COMPREHENSIVE BENCHMARK — SLUTRAPPORT")
    print("=" * 72)
    ...
    print(f"\n  Per svarighetsniva:")
    for lvl in sorted(per_level.keys(), key=int):
        ld = per_level[lvl]
        fd = ld["frankenstein"]
        gd = ld["gemini"]
        d = fd["rate"] - gd["rate"]
        emoji = "🧟" if d > 0 else ("🤖" if d < 0 else "🤝")
        print(f"    Niva {lvl}: Frank {fd['solved']}/{fd['total']} ({fd['rate']:.0%}) vs Gemini {gd['solved']}/{gd['total']} ({gd['rate']:.0%}) {emoji}")

    print(f"\n  Per kategori:")
    for cat in sorted(per_category.keys(), key=lambda c: per_category[c].get("difficulty", 0)):
        cd = per_category[cat]
        fd = cd["frankenstein"]
        gd = cd["gemini"]
        d = fd["rate"] - gd["rate"]
        emoji = "🧟" if d > 0 else ("🤖" if d < 0 else "🤝")
        print(f"    {cat:<20} Nv{cd['difficulty']}: Frank {fd['solved']}/{fd['total']} ({fd['rate']:.0%}) vs Gemini {gd['solved']}/{gd['total']} ({gd['rate']:.0%}) {emoji}")
    ...
    # Save JSON
    json_path = data_dir / f"benchmark_{ts}.json"
    json_path.write_text(json.dumps(output, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"\n  JSON: {json_path}")

    # Save Markdown report
    md_report = _generate_markdown_report(output)
    md_path = data_dir / f"benchmark_{ts}.md"
    md_path.write_text(md_report, encoding="utf-8")
    print(f"  Markdown: {md_path}")

    # Upload to bridge for mobile download
    if bridge_url:
        print(f"\n  Uploading to bridge...")

        # Upload JSON
        json_file_id = _upload_to_bridge(bridge_url, output, f"benchmark_{ts}.json")
        if json_file_id:
        ...
        try:
            ...
            resp = requests.post(
                ...
                json={
                    "data": md_b64,
                    "filename": f"benchmark_{ts}.md",
                    "uploadedBy": "ai",
                    "description": f"Benchmark Report: Frankenstein AI vs Gemini 2.0 Flash",
                    "tags": ["benchmark", "report", "markdown"],
                },
                timeout=10,
            )
            if resp.status_code == 200:
        ...
        # Also send as a chat message so it shows up in mobile
        try:
            msg = (
                f"📊 **Benchmark klar!**\n\n"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Comprehensive Benchmark: Frankenstein AI vs Gemini 2.0 Flash")
    ...
    parser.add_argument("--no-v2", action="store_true", help="Skip V2 tasks (bugfix, api_design, etc)")
    args = parser.parse_args()

    run_comprehensive_benchmark(
        tasks_per_category=args.tasks_per_category,
        bridge_url=args.bridge_url,
        include_v2=not args.no_v2,
    )
