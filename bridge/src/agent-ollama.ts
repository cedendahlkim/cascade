...
export class OllamaAgent {
  ...
  private getTools(): Array<{ type: "function"; function: { name: string; description: string; parameters: { type: string; properties: Record<string, unknown>; required?: string[] } } }> {
    const builtinTools = [
      ...
      { name: "run_command", description: "Run a shell command on the bridge server.", parameters: { type: "object" as const, properties: { command: { type: "string", description: "Command to run" }, cwd: { type: "string", description: "Working directory" }, runner: { type: "string", description: "Runner: host (default) or kali" } }, required: ["command"] } },
