/**
 * DeepSeek AI Agent for Cascade Remote Bridge
 * 
 * Uses DeepSeek's OpenAI-compatible API for chat completions.
 * Participates in Arena research sessions alongside other AI agents.
 * Now with FULL tool support — same capabilities as Claude/Gemini.
 */
...
export type DeepSeekStreamCallback = (chunk: string) => void;
export type DeepSeekStatusCallback = (status: { type: string }) => void;
...
export class DeepSeekAgent {
  ...
  private streamCallback: DeepSeekStreamCallback | null = null;
  private statusCallback: DeepSeekStatusCallback | null = null;
  private tokenUsage: DeepSeekTokenUsage = { inputTokens: 0, outputTokens: 0, totalTokens: 0, requestCount: 0 };
  ...
  constructor(config?: DeepSeekAgentConfig) {
    ...
    this.agentName = config?.name || "DeepSeek";
    this.agentRole = config?.role || "analytiker";
    this.enabled = !!this.apiKey;

    if (this.apiKey) {
      console.log(`[${this.agentName.toLowerCase()}] Initialized with model: ${this.model} (role: ${this.agentRole})`);
    } else {
      console.log(`[${this.agentName.toLowerCase()}] No DEEPSEEK_API_KEY set - agent disabled`);
    }
  }
  ...
  onStream(cb: DeepSeekStreamCallback): void {
    this.streamCallback = cb;
  }

  onStatus(cb: DeepSeekStatusCallback): void {
    this.statusCallback = cb;
  }

  getTokenUsage(): DeepSeekTokenUsage {
  ...
  private getTools(): Array<{ type: "function"; function: { name: string; description: string; parameters: Record<string, unknown> } }> {
    const builtinTools = [
      ...
      { name: "run_command", description: "Run a shell command on the bridge server.", parameters: { type: "object", properties: { command: { type: "string", description: "Command to run" }, cwd: { type: "string", description: "Working directory" } }, required: ["command"] } },
  ...
  async respond(userMessage: string): Promise<string> {
    if (!this.apiKey) {
      return "DeepSeek agent not configured. Set DEEPSEEK_API_KEY in bridge/.env";
    }
        ...
        const resp = await fetch(`${this.baseUrl}/chat/completions`, {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: `Du är ${this.agentName}, en AI-forskare i Cascade Remote Arena. Din roll är "${this.agentRole}".
Du är känd för djup analytisk förmåga, logiskt resonerande och att hitta mönster andra missar.
...
Svara koncist och insiktsfullt. Svara på samma språk som ämnet/frågan.`,
        ...
        if (!resp.ok) {
          ...
          if (resp.status === 402 || errText.includes("Insufficient Balance")) {
            console.error(`[${this.agentName.toLowerCase()}] Insufficient balance — disabling agent`);
            this.enabled = false;
            throw new Error(`DeepSeek: Otillräckligt saldo. Agenten har inaktiverats.`);
          }
      ...
      return finalText || "No response from DeepSeek.";
    } catch (error) {
